...
The exercices for this workshop: [Challenge 1](#challenge1), [challenge 2](#challenge2), [challenge 3](#challenge3), [challenge 4](#challenge4), [challenge 5](#challenge5), [challenge 6](#challenge6), [challenge 7](#challenge7).
...
| ------------                   | -----------  | --------    | -------     | ------      | ---------- |
| Beta	                         | `pbeta`	    | `qbeta`	    | `dbeta`	    | `rbeta`     | $f(x) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) \Gamma(\beta)}, 0 \lt x \lt 1$ |
| Binomial	                     | `pbinom`	    | `qbinom`    | `dbinom`	  | `rbinom`    | $f(x) = {n \choose x} p^x(1-p)^{n-x}, x = 0,1,2,...,n$ |
| Cauchy                         | `pcauchy`    | `qcauchy`   | `dcauchy`	  | `rcauchy`   | $f(x) = \frac{1}{\pi} \frac{1}{x^2+1}, -\infty \lt x \lt \infty$ |
| Chi-Square	                   | `pchisq`	    | `qchisq`    | `dchisq`	  | `rchisq`    | $f(x) = x^{(k/2)-1} \frac{e^{-x/2}}{( 2^{k/2} \Gamma(k/2) )}, x \gt 0$ |
| Exponential	                   | `pexp`       | `qexp`	    | `dexp`	    | `rexp`      | $f(x) = \lambda e^{-\lambda x}$ (when $x \geq 0$, otherwise, 0) |
| F	                             | `pf`	        | `qf`	      | `df`	      | `rf`        | $f(x) = \frac{\Gamma[(r_1+r_2)/2](r_1/r_2)^{r_1/2}}{\Gamma(r_1/2)\Gamma(r_2/2)}\frac{(x)^{r_1/2-1}}{(1+r_1x/r_2)^{(r_1+r_2)/2)}}, x \gt 0$ |
| Gamma	                         | `pgamma`	    | `qgamma`    | `dgamma`	  | `rgamma`    | $f(x) = \frac{\lambda^c x^{c-1}e^{-\lambda x}}{\Gamma(c)}, x \gt 0$ |
| Geometric	                     | `pgeom`	    | `qgeom`	    | `dgeom`	    | `rgeom`     | $f(x) = p(1-p)^{x}, x = 0,1,2,...$ |
| Hypergeometric                 | `phyper`	    | `qhyper`    | `dhyper`	  | `rhyper`    | $f(x) = \frac{{N-D\choose n-x}{D \choose x}}{{N \choose n}}, x = 0,1,2,...,n$ |
| Logistic	                     | `plogis`	    | `qlogis`    | `dlogis`	  | `rlogis`    | $f(x) = \frac{1}{\sqrt{2 \pi \sigma}}e^{\frac{1}{2}(\frac{x-\mu}{\sigma})^2}, -\infty \lt x \lt \infty$ |
| Log Normal	                   | `plnorm`	    | `qlnorm`    | `dlnorm`	  | `rlnorm`    | $f(x)={\frac{1}{x\sigma\sqrt{2\pi}}}e^{-\Bigl(\frac {(ln(x)-\mu)^2}{2\sigma^2}\Bigr)}$ |
| Negative Binomial	             | `pnbinom`    | `qnbinom`   | `dnbinom`	  | `rnbinom`   | $f(x) = {x+r-1 \choose r-1}p^r(1-p)^x, x = 0,1,2,...$ |
| Normal	                       | `pnorm`	    | `qnorm`	    | `dnorm`	    | `rnorm`     | $f(x)={\frac{1}{\sigma\sqrt{2\pi}}}e^{-{\frac{1}{2}}\Bigl(\frac {x-\mu}{\sigma}\Bigr)^2}$ |
| Poisson	                       | `ppois`	    | `qpois`	    | `dpois`	    | `rpois`     | $f(x) = e^{-\lambda}\frac{\lambda^x}{x!}, x = 0,1,2,...$ |
| Student t	                     | `pt`	        | `qt`	      | `dt`	      | `rt`        | $f(x) = \frac{\Gamma[(r+1)/2]}{\sqrt{\pi r}\Gamma(r/2)}\frac{1}{(1+x^2/r)^{(r+1)/2}}$ |
...
| Weibull	                       | `pweibull`	  | `qweibull`  | `dweibull`  | `rweibull`  | $f(x) = \frac{k}{\alpha} \frac{(x-\mu)^{k-1}}{\alpha}e^{-((x-\mu)/\alpha)^k}, x \geq \mu;k.\alpha \gt 0$ |
...
| ------------ | ----------- | -------- | ------- | ------ | ---------- |
| Beta	       | `pbeta`	 | `qbeta`	| `dbeta`	| `rbeta` | $f(x) = \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) \Gamma(\beta)}, 0 \lt x \lt 1$ |
| <span style="background-color: #FFFF00">**Binomial**</span>	   | `pbinom`	 | `qbinom`	| `dbinom`	| `rbinom` | $f(x) = {n \choose x} p^x(1-p)^{n-x}, x = 0,1,2,...,n$ |
| Cauchy       | `pcauchy` | `qcauchy` | `dcauchy`	| `rcauchy` | $f(x) = \frac{1}{\pi} \frac{1}{x^2+1}, -\infty \lt x \lt \infty$ |
| <span style="background-color: #FFFF00">**Chi-Square**</span>	 | `pchisq`	 | `qchisq`	 | `dchisq`	| `rchisq` | $f(x) = x^{(k/2)-1} \frac{e^{-x/2}}{( 2^{k/2} \Gamma(k/2) )}, x \gt 0$ |
| <span style="background-color: #FFFF00">**Exponential**</span>	 | `pexp`    | `qexp`	 | `dexp`	| `rexp` | $f(x) = \lambda e^{-\lambda x}$ (when $x \geq 0$, otherwise, 0) |
| <span style="background-color: #FFFF00">**F**</span>	           | `pf`	     | `qf`	| `df`	| `rf` | $f(x) = \frac{\Gamma[(r_1+r_2)/2](r_1/r_2)^{r_1/2}}{\Gamma(r_1/2)\Gamma(r_2/2)}\frac{(x)^{r_1/2-1}}{(1+r_1x/r_2)^{(r_1+r_2)/2)}}, x \gt 0$ |
| Gamma	       | `pgamma`	 | `qgamma`	| `dgamma`	| `rgamma` | $f(x) = \frac{\lambda^c x^{c-1}e^{-\lambda x}}{\Gamma(c)}, x \gt 0$ |
| Geometric	   | `pgeom`	 | `qgeom`	| `dgeom`	| `rgeom` | $f(x)=p(1-p)^{x}, x = 0,1,2,...$ |
| Hypergeometric | `phyper`	| `qhyper`	| `dhyper`	| `rhyper` | $f(x) = \frac{{N-D\choose n-x}{D \choose x}}{{N \choose n}}, x = 0,1,2,...,n$ |
| <span style="background-color: #FFFF00">**Logistic**</span>	     | `plogis`	| `qlogis`	| `dlogis`	| `rlogis` | $f(x) = \frac{1}{\sqrt{2 \pi \sigma}}e^{\frac{1}{2}(\frac{x-\mu}{\sigma})^2}, -\infty \lt x \lt \infty$ |
| <span style="background-color: #FFFF00">**Log Normal**</span>	   | `plnorm`	| `qlnorm`	| `dlnorm`	| `rlnorm` | $f(x)={\frac{1}{x\sigma\sqrt{2\pi}}}e^{-\Bigl(\frac {(ln(x)-\mu)^2}{2\sigma^2}\Bigr)}$ |
| Negative Binomial	| `pnbinom`	| `qnbinom`	| `dnbinom`	| `rnbinom` | $f(x) = {x+r-1 \choose r-1}p^r(1-p)^x, x = 0,1,2,...$ |
...
| <span style="background-color: #FFFF00">**Poisson**</span>	     | `ppois`	| `qpois`	| `dpois`	| `rpois` | $f(x) = e^{-\lambda}\frac{\lambda^x}{x!}, x = 0,1,2,...$ |
| <span style="background-color: #FFFF00">**Student t**</span>	   | `pt`	| `qt`	| `dt`	| `rt` | $f(x) = \frac{\Gamma[(r+1)/2]}{\sqrt{\pi r}\Gamma(r/2)}\frac{1}{(1+x^2/r)^{(r+1)/2}}$ |
| Studentized Range | `ptukey`	| `qtukey`	| `dtukey`	| `rtukey` | You just don't want to see it... |
...
- The function `choose()` is used in R to find the number combinations (regardless of the order). For example, if you just want to know the number of combinations you can have from choosing 6 numbers out of 49, it is `r choose(49, 6)`. If you wish to know the chances of winning the lotteries it's `r 1/choose(49, 6)`.
...
```{r binom_chall_coin, fig.width=8,fig.height=4}
...
In particular, thank to the Barrett lab graduate students for their valuable insights and support, Winer Daniel Reyes Corral, etc. 
