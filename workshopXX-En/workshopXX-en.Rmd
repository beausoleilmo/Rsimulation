...
- .alert[NOTE:] the probability of an event occurring *is not the same* as the p-value, since the p-value is the sum of 3 parts. We add part 2. and 3. because it gives us information about the *specialness* of this rare event.  In other words, we want to know if the event we measured is statistically special or, as you might have heard it, statistically significant!
...
Find the "x" value (quantile) based on the probability (area under the curve between 2 values)

```{r Normal_pdf_important_values2}
qnorm(p = c(.75, .95,.975, .995),lower.tail = F)
```
- for a probability of $50\%$ between the "x" value of a standard normal is `r round(qnorm(p = c(.75, .25),lower.tail = F),2)`. 
- for a probability of $95\%$ between the "x" value of a standard normal is `r round(qnorm(p = c(.95, .05),lower.tail = F),2)`. 
- for a probability of $97.5\%$ between the "x" value of a standard normal is `r round(qnorm(p = c(.975, .025),lower.tail = F),2)`. 
- for a probability of $99.5\%$ between the "x" value of a standard normal is `r round(qnorm(p = c(.995, .005),lower.tail = F),2)`. 
