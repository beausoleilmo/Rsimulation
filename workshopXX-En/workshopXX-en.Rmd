...
???
- and models are like maps: they are an abstraction of the real worl. It is not the real world. No same person thinks a map is "right". The only map that is right is the 1:1 real map. But this map woulb not be interesting to get a sense of how the world is. 
...
???
- Here, we are coming back to the idea of looking at patter in random processes. 
...
par(mfrow=c(2,2), cex =1.1) # set window 
...
- We believe that we have a coin, which has a 75% chance of landing on 'heads':
...
par(mfrow = c(1,3), cex = 1.2); r.x = range(c(x.lm,rowMeans(sim.lm),fitted(lm.out)))
...
- Same as the simulation above, but now we plot the adjusted $R^2$ from the linear model. 
...
```
- You can see from the plots that the effect size (the $R^2$ in this case), is extremely variable when the sample size is very small. But when the sample size increases, the spread around the effect size diminishes. This adding to the idea that larger sample sizes are more important when you conduct a study. 
