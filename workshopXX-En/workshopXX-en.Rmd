...
```{r setup, echo = F}
# in the end, should be abour 1500 lines
...
See the [QCBS R workshops](https://r.qcbs.ca/fr/). 

---
## Learning objectives

1. Develop intuition and skills to perform simulations
2. Use functions useful when doing simulations 
...
- Therefore, since we designed the simulation, we know the processes that underlie the data generated
- It is then possible to test various methods to 
  1. see if they work, 
  2. do power analysis, 
  3. verify assumptions
  4. learn how data is generated 
  5. etc. 
  
- The idea here is that perhaps we are interested in understanding how a certain process was generated (how X influences Y, which could be written as X -> Y or Y~X). 
...
- When performing simulations, once has to keep in mind 
  1. the type of distribution underlying the data of interest
  2. the parameters of the distribution itself (mean, standard deviation, rate, degrees of freedom, etc.)
  3. the statistical model making the relationship between the response variable and the explanatory variable. 
  4. what we want to show or learn with the simulation

---
## Tips when performing simulation

.alert[Description section]. Add a 'Description' section to all your simulation scripts to introduce what the script is about. You can add information how to use important arguments or a step-by-step description on how to use the script. You can add some references that you used to build your scrip. 
...
---
## Fucntions useful in simulations (RNG)
...
---
## Fucntions useful in simulations (sample)
...
```{r}
set.seed(12345) # Sets the random nubmer generator to a fix value
vec.1.10 = 1:10 # Make the vector to chose from 
...
plot(x, dnorm(x, 0, 1), main = "Density normal", type = "l", lwd = 2, col = "blue", ylab = "", xlab = "x", ylim = c(0,1))
...
text(x = ub, y = .2,labels = paste0(round(qnorm(p),2)),adj = 0,pos = 1,offset = -1)
...
text(x = lb, y = .2,labels = paste0(round(qnorm(1-p),2)),adj = 0,pos = 1,offset = -1)
...
text(x = mean(x2), y = .2,labels = paste0((1-2*p)*100,"%"),adj = 0,pos = 1)

text(x = 0, y = .55,labels = paste0(quote("Mean")),adj = 0,pos = 1,offset = 0)
...
- Now that we have the building blocks to make simulations, we are going to add another layer:
  ...
  - For this, we need a certain workflow that resemble this: 
...
sample.mean.pop.est <- function(x,n, sample.size, ylim = NULL) {
  ...
  # sample.size: how big is the sample size from which the MEAN will calculated from 
