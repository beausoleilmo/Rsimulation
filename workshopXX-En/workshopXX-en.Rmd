...
hypothesis.testing <- function(mean.pop=2, sd = 1, n=NULL) {
...
curve(dnorm(x, mean = mean, sd = sd),-6,10, xlim = c(-5, 7), ylim = c(0,1),xlab = "", lwd =3 )
text(x = -4, y = .25,labels = "Sampling \ndistribution \nif H0 is true",adj = 0,pos = 4, cex=cex)
arrows(x0 = -3,x1 = -2, y0 = .2,y1 = .1,code = 2,length=.1)

null.test =qnorm(p = .05, mean = mean, sd=sd,lower.tail = F)
abline(v = null.test)



# Add the right (green) polygon 
lb <- qnorm(pnorm(null.test,mean = mean),mean) # Lower bound
ub <- max(x)   # Upper bound
x2 <- seq(lb, ub, length = 100) # New Grid
...
curve(dnorm(x, mean = mean2, sd = sd),-6,10, xlim = c(-5, 8), ylim = c(0,0.6),col = "red",add = T, lwd =3 )
