...
- "responsible about the conscious choices we need to make in order to get an answer.": 
  * What I mean here is that when we design a simulation, we have to build in the assumptions of the models or the process we are trying to recreate. So we need to *decide* (or plan), what is going to make up the process. In the simulation, the parameters and variable are chosen by you, but they need to be plausible (from the literature, or maybe a collegue could help you on this with data that has already been collected).
...
- It is then possible to test various methods or analysis to 
    ...
    5. "see" what a mathematical model or equation predicts,
    6. etc. 
...
- Log (natural log, or ln) odds can range from $-\infty$ to $\infty$ and makes the odds symetric. It is calculated as $ln (odds) = ln(p/(1-p))$ (i.e., the logit function).
...
.tiny[
```{r odds_prob_plot, fig.width=12,fig.height=4}
...
- .alert[NOTE:] the probability of an event occurring *is not the same* as the p-value, since the p-value is the sum of 3 parts. 
...
- for a probability of $95\%$ between the "x" value is `r round(qnorm(p = c(.95, .05),lower.tail = F),2)`. 
- for a probability of $97.5\%$ between the "x" value is `r round(qnorm(p = c(.975, .025),lower.tail = F),2)`. 
- for a probability of $99.5\%$ between the "x" value is `r round(qnorm(p = c(.995, .005),lower.tail = F),2)`. 
...
# hist(sum.rdm.var/i, main = paste("Histogram of",i,"variables"), xlab = "x")
...
```{r sim_lm_cat_plot, fig.width=8,fig.height=5}
par(mfrow=c(1,2))
...
```{r sim_lm_interaction_discrete, echo=FALSE, fig.width=8,fig.height=5}
set.seed(1)
n = 100
...
virPal <- viridis::viridis(length(x2.sim))
...
plot(x = df.lm$x1, y = df.lm$y, col = df.lm$x2.col, 
     # xlim=range(x2.sim),
     pch = 19, xlab = "x1", ylab = "y")
apply(eff.dat, 1, function(x) abline(a = x[3], b = x[2], col = x[4]))
abline(h = 0, v = 0,lty = 3)
legend("topleft", title = "x2",legend = round(eff.dat$x2.sim,1), lty = 1,col = eff.dat$x2.col, bg = scales::alpha("white",.5))
# summary(lm.out)
```
...
x1z = scale(x1)
x2z = scale(x2)
z = 1 + 2*x1z + 3*x2 # this is the equation that gives the LOG odds. Note $p = odds/(1+odds)$
...
polygon = list(matrix(c(2, 1, 3, 3, 2, 5, 
                        3, 7, 2, 9, 1, 7, 
                        0, 5, 1, 3, 2, 1), ncol=2, byrow=T)) 
