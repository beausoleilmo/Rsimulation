...
```{r uniform_distcrete_fun_dist, echo=TRUE, fig.width=5,fig.height=3}
# Define uniform discrete 
dunifdisc<-function(x, min=0, max=1) ifelse(x>=min & x<=max & round(x)==x, 1/(max-min+1), 0)
runifdisc<-function(n, min=0, max=1) sample(min:max, n, replace=T)
curve(dunifdisc(x, 7,10), type = "h",from=6, to=11, col="black", 
      lwd=1, ylim = c(0,.5), ylab = "Probability", main = "Uniform") 
```
...
```{r uniform_cont_angles, echo=TRUE, fig.width=5,fig.height=4}
x = seq(0,360,by = .1)
y <- dunif(x, min = min(x),max = max(x))
plot(y~x, type = "n", ylim = c(0,1.5*max(y)), ylab = "Density", main = "Uniform")
polygon(c(x, rev(x), 0), c(y, rep(0,length(y)), 0), col="blue")
```
