...
```{r odds_prob_plot, echo=FALSE, eval=FALSE, fig.width=12,fig.height=8}
...
```{r odds_prob_plot_with_pol, echo=FALSE, eval=TRUE, fig.width=12,fig.height=8}
# adde prob scale
par(mfrow = c(2,3))
par(mar = c(0,4,0,0), 
    # mfrow=c(1,2),
...
- see [Probability Cheat Sheet2](https://github.com/jfieberg/probability_cheatsheet) for more details
- see [Probability Cheat Sheet3](https://github.com/wzchen/probability_cheatsheet) for more details
]
...
![Relationship between statistical distributions:scale 90%](images/Relationships_among_some_of_univariate_probability_distributions.jpg)
???
...
```{r dbinom_plot_ex, echo=-c(1:4), fig.width=8,fig.height=4}
par(mfrow = c(1,1), mar = c(4,4,1,1))
...
.pull-left4[
```{r lm_assumtion_expected_mean, echo=FALSE, fig.width=5,fig.height=5}
par(mar=c(4,4,0.1,0.1))
set.seed(12345678)
...
.pull-right4[
```{r lm_assumtion_expected_mean_interactive, echo=FALSE, fig.width=5,fig.height=4.5}
library(plotly)
library(dplyr)
...
| Type | Equation |
| ---- | -------- |
| Linear | $Y = \beta_{0} + \beta_{1} x_{1}  + \epsilon$ or $\mu = \beta_{0} + \beta_{1} x_{1}$ for $Y = Normal(\mu, \sigma)$ |
...
```{r simulate_lm_resimulate_from_model, echo=-1, fig.width=8,fig.height=3}
par(mfrow = c(1,3), mar = c(4,4,1,1), cex = 1.2)
...
```{r simulate.gaussian.model, echo=FALSE, fig.width=8,fig.height=6}
...
```{r lm_simulate_slope, echo=FALSE, fig.width=8,fig.height=3}
...
```{r lmm_sim.blog, fig.width=8, fig.height=3}
set.seed(16)
nstand = 5
nplot = 4
mu = 10
...
```{r rdm_pts_in_polygon, echo=FALSE}
...
```{r rdm_pts_in_polygon_grid, echo=FALSE}
set.seed(456)
...
.tiny[
Book
- Aho, K. A. 2013. Foundational and applied statistics for biologists using R.
- Banks, J. 1998. Handbook of simulation.
- Bloomfield, V. A. 2009a. Using R for computer simulation and data analysis in biochemistry, molecular biology, and biophysics
- Bloomfield, V. A. 2009b. Computer simulation and data analysis in molecular biology and biophysics: an introduction using R. 
- Bolker, B. M. 2015. Ecological Models and Data in R. 
- Crawley, M. J. 2013. The R book. Pages 1–1051. 2nd ed. (see chapter 7)
- Dobrow, R. P. 2013. Probability: With Applications and R. 
- Jones, O., R. Maillardet, and A. Robinson. 2014. Introduction to scientific programming and simulation using R. 2nd ed.
- Morgan, B. J. T. 1984. Elements of simulation.
- Otto, S. P., and T. Day. 2007. A Biologist's Guide to Mathematical Modeling in Ecology and Evolution.
- Ross, K. 2021. An introduction to probability and simulation.
- Seefeld, K., and E. Linder. 2007. Statistics using R with biological examples.
- Soetaert, K., J. Cash, and F. Mazzia. 2012. Solving Differential Equations in R.
- Templ, M. 2016. Simulation for data science with R. 
- Whitlock, M. C., and D. Schluter. 2020. The analysis of biological data. 3rd ed.
