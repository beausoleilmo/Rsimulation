...
<!-- For developers  -->
<!-- 
Please consult the presentation template at the following link  http://r.qcbs.ca/templateWorkshops/presentation-template/index.html#1
-->
...
- So from the mean of the standard normal distribution, if you are at $1 \times \sigma$ from both sides of the mean, you have about `r paste0(p.from.mean[1],"%")` of the data. At $2 \times \sigma$ it's `r paste0(p.from.mean[2],"%")` and at $3 \times \sigma$ it's `r paste0(p.from.mean[3],"%")`
...
draw.normal(where = "left",  prob = .25); x.val = qnorm(p = c(.25),lower.tail = T)
arrows(x0 = x.val, x1 = x.val, y0 = .5,y1 = .4, length = length.arrow.head)
text(x.val, y = .6, labels = round(x.val,2))

draw.normal(where = "left",  prob = .05); x.val = qnorm(p = c(.05),lower.tail = T)
...
draw.normal(where = "left",  prob = .005); x.val = qnorm(p = c(.005),lower.tail = T)
arrows(x0 = x.val, x1 = x.val, y0 = .5,y1 = .15, length = length.arrow.head)
text(x.val, y = .6, labels = round(x.val,2))
